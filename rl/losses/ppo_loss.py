...
def create_ppo_value_and_grad_fn(
    policy_model: nn.Module,
    value_model: nn.Module,
    config: PPOConfig = None
):
    """
    Create a value_and_grad function for PPO training.

    Returns a function compatible with MLX's optimization loop.
    """
    if config is None:
        config = PPOConfig()

    def loss_fn(batch: Dict) -> Tuple[mx.array, Dict]:
        return compute_ppo_loss_for_batch(
            policy_model, value_model, batch, config
        )
