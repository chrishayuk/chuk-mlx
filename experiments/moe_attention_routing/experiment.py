...
class AttentionRouterExperiment(ExperimentBase):
    ...
    def _evaluate_hypotheses(self, all_results: dict) -> dict:
        ...
        # Extract attention ratios (excluding L0 which has 0% by construction)
        model_ratios = {}
        for model_name, results in all_results.items():
            if "error" in results:
                continue
            ...
            # Exclude L0 from evaluation - it has 0% by construction
            layer_ratios = [
                data.get("attention_ratio", 0)
                for layer_name, data in layers.items()
                if layer_name != "L0"
            ]
            if layer_ratios:
                ratio = sum(layer_ratios) / len(layer_ratios)
            else:
