...
We investigate whether attention-driven routing is universal across MoE architectures. By decomposing the router input signal into token embedding and attention contributions, we find that **attention dominates routing in OLMoE just as it does in GPT-OSS**. At middle layers, 89% of the routing signal comes from attention; at late layers, this rises to 98%. Combined with 78% context sensitivity (same token routing to different experts based on context), this suggests the MoE router is architecturally redundant across all MoE types—it merely reads a decision that attention has already made.

## 1. Introduction

### 1.1 Background

Previous analysis of GPT-OSS revealed a striking finding: 96% of the router's input signal comes from attention output, with token embeddings contributing only 4%. This raised a fundamental question:

**Is attention-driven routing universal across different MoE architectures?**

Different MoE models have different expert structures. If the routing mechanism differs between architectures, it would have significant implications for MoE optimization and compression.
...
### 3.4 Comparison with GPT-OSS

| Metric | GPT-OSS | OLMoE |
|--------|---------|-------|
| Gate rank | 48.9% of max | 74% of max |
| Experts | 32 | 64 |
| Middle layer attention ratio | ~96% | 89% |
| Late layer attention ratio | ~96% | 98% |
| Context sensitivity | High | High (78%) |

The patterns are **remarkably similar** across both architectures, with attention dominating routing in both cases.
...
**Router Redundancy**: The MoE router appears to be an architectural redundancy in **all** MoE designs. It performs a linear readout of a decision that attention has already encoded in the hidden state.
...
**Generalization**: Findings from GPT-OSS analysis—such as cold expert identification and workhorse concentration—likely apply to other MoE architectures as well.

### 4.4 Limitations

1. **Limited model testing**: We tested OLMoE and GPT-OSS; other MoE architectures (Mixtral, Switch Transformer) should be validated.
...
In OLMoE, attention contributes 89% of the routing signal at middle layers and 98% at late layers—matching the pattern observed in GPT-OSS. The router does not perform meaningful integration of token and context signals; it simply reads the decision that attention has already made.

This finding:
- Confirms that router redundancy is universal across MoE architectures
