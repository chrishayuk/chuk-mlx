...
def test_compression_level(
    model, tokenizer, compressor, layer_idx, target_experts, baseline_outputs, prompts
):
...
def main():
    ...
    for target in compression_levels:
        ...
        for layer_idx in hooks.moe_layer_indices:
            plan = compressor.plan_compression(
                layer_idx, target_experts=target, strategy="aggressive"
            )
            plans.append(plan)
        ...
        print(
            f"  Size: {results[target]['original_b']:.2f}B -> {results[target]['compressed_b']:.2f}B"
        )
    ...
    for target in compression_levels:
        r = results[target]
        print(
            f"{target:<10} {r['original_b']:.2f}B{'':<6} {r['compressed_b']:.2f}B{'':<6} {r['reduction_pct']:.1f}%{'':<7} {r['quality_pct']:.0f}%"
        )
    ...
    for target in compression_levels:
        ...
        if r["quality_pct"] >= 80:  # Acceptable quality threshold
            if best is None or abs(r["reduction_pct"] - 50) < abs(
                results[best]["reduction_pct"] - 50
            ):
                best = target
