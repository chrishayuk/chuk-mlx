...
class OrthogonalExtractionAnalyzer:
    ...
    def generate_with_steering(
        self, prompt: str, steering_vector: np.ndarray, layer_idx: int, strength: float
    ) -> str:
    ...
    def test_orthogonality(self, directions: dict) -> dict:
        ...
        for layer in self.target_layers:
            ...
            same_digit = np.array(
                [np.dot(a_vecs_norm[i], b_vecs_norm[i]) for i in range(len(self.digits))]
            )
            ...
            print(
                f"{'A_i vs A_j (diff digits)':<30} {a_vs_a_mean:>12.3f}   {interpret(a_vs_a_mean)}"
            )
            print(
                f"{'B_i vs B_j (diff digits)':<30} {b_vs_b_mean:>12.3f}   {interpret(b_vs_b_mean)}"
            )
            print(
                f"{'A_i vs B_j (cross-role)':<30} {a_vs_b_mean:>12.3f}   {'ORTHOGONAL' if abs(a_vs_b_mean) < 0.3 else 'NOT orthogonal'}"
            )
            print(
                f"{'A_i vs B_i (same digit)':<30} {same_digit_mean:>12.3f}   {'Role > Digit' if same_digit_mean < 0.5 else 'Digit > Role'}"
            )
    ...
    def extract_product_directions(self) -> dict:
        ...
        for layer in self.target_layers:
                ...
                if len(pairs) > 1:
                    ...
                    for i, (a1, b1) in enumerate(pairs):
                        for a2, b2 in pairs[i + 1 :]:
                            ...
                            sim = np.dot(act1, act2) / (
                                np.linalg.norm(act1) * np.linalg.norm(act2) + 1e-10
                            )
            ...
            if same_product_mean > same_operand_mean + 0.05:
                print("  → PRODUCT-INDEXED lookup (1D)")
            elif same_operand_mean > same_product_mean + 0.05:
                print("  → OPERAND-INDEXED lookup (2D)")
            else:
                print("  → Mixed/distributed encoding")
    ...
    def test_operand_steering(self, directions: dict) -> dict:
        ...
        # Get baseline
        baseline_output = self.generate_with_steering(
            base_prompt, np.zeros(self.hidden_size), 20, 0
        )
    ...
    def run_all_phases(self) -> dict:
        ...
        results["directions"] = {
            "a_directions": {
                str(d): {str(l): dirs[l].tolist() for l in self.target_layers}
        ...
        results["products"] = {
            "directions": {
                str(p): {str(l): dirs[l].tolist() for l in self.target_layers}
