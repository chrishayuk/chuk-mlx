...
class MockFineTuneBatchDataset:
    ...
    def _generate_mock_batches(self):
        ...
        for i in range(self.num_batches):
            ...
            concatenated_tensor = np.concatenate((input_tensor, target_tensor), axis=1)
            batch_file = f"{self.batchfile_prefix}_{i}.npz"
            batch_path = os.path.join(self.batch_output_dir, batch_file)
            np.savez(batch_path, concatenated_tensor=concatenated_tensor, lengths=lengths)
    ...
    def __getitem__(self, index):
        ...
        return concatenated_tensor['concatenated_tensor'], concatenated_tensor['lengths']
